...
print("Number of Subjects (SWI Contrast):",len(ROI_total))
...
for i in range(len(ROI_total)):
    ...
    for j in range(len(ROI_total['N%s'%(i+1)])):
        ROI.append(ROI_total['N%s'%(i+1)][j])

#ROI: 74 SWI CMBs. 

#We will partition 74 into 56 train / 18 test.
...
class FakeDataset3D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = torch.randint(0, self.num_classes, size = (27,27,27), dtype = torch.long)
...
#0.B SWI dataset for CMB    

class TrainDataset(data.Dataset):
    def __init__(self, size = 56, image_size = (1,16,16,16), num_classes = 2, transform = None, target_transform = None, random_offset =0):
...
class M1(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv3d(1,64, (5,5,5)), nn.Dropout(0.2)) # in_channel, out_channel, kernel
        ...
        self.conv3 = nn.Sequential(nn.Conv3d(64, 64, (3,3,3)), nn.Dropout(0.3)) # in_channel, out_channel, kernel
